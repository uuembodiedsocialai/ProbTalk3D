...
parser = argparse.ArgumentParser(description="FLAME model", allow_abbrev=True)

parser.add_argument(
    "--flame_model_path",
    type=str,
    default=Path(f"{parent_dir}/model/generic_model.pkl"),
...
parser.add_argument(
    ...
    type=str,
    default=Path(f"{parent_dir}/model/flame_static_embedding.pkl"),
...
parser.add_argument(
    ...
    type=str,
    default=Path(f"{parent_dir}/model/flame_dynamic_embedding.npy"),
...
def get_config(batch_size=None, **kwargs):
    # config = parser.parse_args()
    config, _ = parser.parse_known_args()

    # Update config with additional arguments if they match existing argparse arguments
    for key, value in kwargs.items():
        if hasattr(config, key):
            setattr(config, key, value)
        else:
            print(f"Ignoring unrecognized parameter: {key}")
