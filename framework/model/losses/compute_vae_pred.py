...
class ComputeLosses(Module):
    def __init__(self,  **kwargs):
        ...
        self._params = {loss: kwargs[loss] for loss in losses if loss not in ["total"]}

    def update(self, latent_pred=None, latent_ref=None, motion_pred=None, motion_ref=None):
        total: float = 0.0
        total += self._update_loss("latent_manifold", outputs=latent_pred, inputs=latent_ref)
