# Adapted from the code of learning2listen https://github.com/evonneng/learning2listen
from torch.nn import functional as F
from einops.layers.torch import Rearrange

from framework.model.utils.transformer_module_util import *
...
class PositionEmbedding(nn.Module):
  ...
  def __init__(self, max_sequence_len, dim):
    super().__init__()
    self.pos_embedding = nn.Parameter(torch.zeros(max_sequence_len, dim))
...
class CrossModalLayer(nn.Module):
  ...
  def __init__(self, **kwargs):
    ...
    self.transformer_layer = Transformer(
        in_size=kwargs['hidden_size'],
        hidden_size=kwargs['hidden_size'],
        num_hidden_layers=kwargs['num_hidden_layers'],
        num_attention_heads=kwargs['num_attention_heads'],
        intermediate_size=kwargs['inter_size'])

    self.cross_norm_layer = nn.LayerNorm(kwargs['in_dim'])
    self.cross_output_layer = nn.Linear(kwargs['in_dim'],
                                        kwargs['out_dim'],
                                        bias=False)
    self.cross_pos_embedding = PositionEmbedding(kwargs['sequence_length'], kwargs['in_dim'])
  ...
  def forward(self, modal_a_sequences, modal_b_sequences, mask_info):
    ...
    modal_a_sequences : tensor
        the first modality (e.g. motion embedding)
    modal_b_sequences : tensor
        the second modality (e.g. audio embedding)
    mask_info: dict
